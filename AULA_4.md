...
```mermaid
graph TD
    A[Objetivos de Aprendizagem - Aula 4] --> B[Saber: Conhecimentos]
    A --> C[Saber Fazer: Habilidades Práticas]
    A --> D[Saber Ser: Atitudes & Cidadania]
    
    B --> B1[Conectar hardware, sistema de pastas e atalhos vitais]
    B --> B2[Compreender a lógica matemática e fórmulas de planilhas]
    B --> B3[Dominar a estrutura de prompts e recursos visuais de IA]
    
    C --> C1[Digitar com postura correta e coordenação motora confiante]
    C --> C2[Montar planilha orçamentária com SOMA, MÉDIA e moeda R$]
    C --> C3[Gerar slogan e logotipo digital com ChatGPT e Bing Creator]
    C --> C4[Montar apresentação express no Gamma ou slides visuais]
    
    D --> D1[Superação definitiva do medo de errar ou quebrar o PC]
    D --> D2[Orgulho da própria trajetória e reconhecimento de conquistas]
    D --> D3[Autonomia para seguir usando a tecnologia no dia a dia]
```
