...
```mermaid
graph TD
    A[Objetivos de Aprendizagem - Aula 3] --> B[Saber: Conhecimentos]
    A --> C[Saber Fazer: Habilidades]
    A --> D[Saber Ser: Atitudes & Cidadania]
    
    B --> B1[O que é IA no cotidiano e como ela pensa]
    ...
    C --> C5[Estudar com fontes e PDFs no Google NotebookLM e WeLib]
    
    D --> D1[Perder o medo de errar ou quebrar a máquina]
    D --> D2[Praticar empatia e solidariedade intergeracional]
    D --> D3[Desenvolver olhar crítico contra fake news e desinformação]
```
...
```mermaid
graph LR
    DUA[Princípios do DUA na Oficina] --> R1[Múltiplas Formas de Representação]
    DUA --> R2[Múltiplas Formas de Ação e Expressão]
    DUA --> R3[Múltiplas Formas de Engajamento]
    
    R1 --> R1A[Textos + Ícones + Voz + Demonstração no Telão]
    R2 --> R2A[Digitar, Falar no Microfone ou Desenhar]
    R3 --> R3A[Escolha de temas pessoais + Parcerias solidárias]
```
